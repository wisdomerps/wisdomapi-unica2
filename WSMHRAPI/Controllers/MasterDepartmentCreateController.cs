...
{
    public class MasterDepartmentCreateController : ApiController
    {
        ...
        public IHttpActionResult Post(MasterDepartmentModel MD)
        {
            try
            {
                if (MD.FTDeptCode != "")
                {
                    ...
                    //VerrifyData 
                    if (MasterClass.VerrifyDuplication(_table, PK, MD.FTDeptCode, MD.FNHSysCmpId))
                    {
                        //create
                        if (MasterClass.CreateMaster(_table, MD.FTDeptCode, _sysCmpId, PK, MD.FTDeptDescTH, MD.FTDeptDescEN, MD.FTRemark, MD.username, 0, ref fnhsysmasterId, ref msgDesc))
                        {
                            a.Status = true;
                            a.StatusCode = 200;
                            a.Messege = msgDesc;
