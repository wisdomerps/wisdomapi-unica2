...
{
    public class MasterCompanyCreateController : ApiController
    {
        ...
        public IHttpActionResult Post(MasterCompanyModel M)
        {
            try
            {
                if (M.FTCmpCode != "")
                {
                    ...
                    //VerrifyData 
                    if (MasterClass.VerrifyDuplication(_table, PK, M.FTCmpCode, M.FNHSysCmpId))
                    {
                        //create
                        if (MasterClass.CreateMaster(_table, M.FTCmpCode, _sysCmpId, PK, M.FTCmpNameTH, M.FTCmpNameEN, M.FTRemark, M.username, 0, ref fnhsysmasterId, ref msgDesc))
                        {
                            a.Status = true;
                            a.StatusCode = 200;
                            a.Messege = msgDesc;
