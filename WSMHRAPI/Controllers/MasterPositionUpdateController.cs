...
{
    public class MasterPositionUpdateController : ApiController
    {
        ...
        public IHttpActionResult Post(MasterPositionModel M)
        {
            try
            {
                if (M.FTPositCode != "")
                {
                    ...
                    {
                         //VerrifyData  
                        if (MasterClass.VerrifyDuplication(_table, PK, M.FTPositCode, M.FNHSysCmpId, M.FNHSysPositId))
                        {
                            //verify Use

                            //update
                            if (MasterClass.UpdateMaster(_table, M.FTPositCode, M.FNHSysCmpId, PK, M.FTPositNameTH, M.FTPositNameEN, M.FTRemark, M.username, M.FNEmployeeFormatType, M.FNHSysPositId, M.FTStateActive, ref msgDesc))
                            {
                                a.Status = true;
                                a.StatusCode = 200;
                                a.Messege = msgDesc;
